...
export default function AnchorVPNUserDashboard() {
    const [activeTab, setActiveTab] = useState<"certs" | "alerts">("certs");
    const [certs, setCerts] = useState<Certificate[]>([]);
    const [loading, setLoading] = useState<boolean>(false);
    const [error, setError] = useState<string | null>(null);
    const [orgId, setOrgId] = useState<string>(() => localStorage.getItem("orgId") || "1");
    ...
    const renderCertTable = (data: Certificate[]) => (
                    ...
                    {data.map((ca) => {
                        ...
                        return (
                            <tr key={ca.id} className="hover:bg-gray-50">
                                <td className="border p-2">{ca.id}</td>
            ...
            <main className="max-w-6xl mx-auto mt-8 bg-white p-8 rounded-xl shadow-sm border border-gray-200">
                ...
                {!loading && !error && (
                    <>
                        {activeTab === "certs" && (
                            <>
                                <h2 className="text-sm mb-4 text-blue-700 text-center">
                                    My Certificates
                                </h2>
                                {certs.length > 0
